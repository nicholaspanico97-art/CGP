...
4. ~~**A way to play it**~~ — **done in v1.5**: `sim/game.py`,
   `sim/play.py`. Plain text, deliberately. Per Nick, Sep 13: play testing
   is its own calibration, so **next is whatever playing it finds.**
